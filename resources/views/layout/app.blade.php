...
<body>
    ...
    <script>
        ...
        localStorage.setItem('token', @json(session('token')));
    </script>
    @endif
    ...
    <script>
        ...
        document.addEventListener('logout', function () {
            ...
            sessionStorage.removeItem('lastPeriodicUpdate');
        ...
        <aside class="left-sidebar">
            ...
            <div>
                <div class="brand-logo d-flex align-items-center justify-content-between">
                    <a href="/"
                        ...
                        <img src="{{ asset('assets/images/logos/logo_gabah.png') }}" width="40" height="40"
                            alt="">
                        <span class="mb-0"
                            style="color: #1E3B8A; font-weight: 800; font-size: 23px;">GrainDryer</span>
                    </a>
                ...
                <nav class="sidebar-nav scroll-sidebar" data-simplebar="">
                    <ul id="sidebarnav">
                        @if (in_array('view dashboard', session('user_permissions', [])))
                        ...
                        @endif
                        @if (in_array('view proses pengeringan', session('user_permissions', [])) ||
                                in_array('view validasi', session('user_permissions', [])))
                            <li class="nav-small-cap" style="margin-left: -10px;">
                                <i class="ti ti-dots nav-small-cap-icon fs-4"></i>
                        ...
                        @if (in_array('view proses pengeringan', session('user_permissions', [])))
                            <li class="sidebar-item">
                                <a class="sidebar-link {{ request()->is('prediksi') ? 'active' : '' }}" href="/prediksi"
                                    aria-expanded="false">
                                    <span>
                        ...
                        @endif
                        {{-- @if (in_array('view validasi', session('user_permissions', [])))
                            <li class="sidebar-item">
                                <a class="sidebar-link {{ request()->is('validasi') ? 'active' : '' }}" href="/validasi"
                                    aria-expanded="false">
                                    <span>
                        ...
                        @if (in_array('view data jenis gabah', session('user_permissions', [])) ||
                                in_array('view data perangkat', session('user_permissions', [])) ||
                                in_array('view role', session('user_permissions', [])))
                            <li class="nav-small-cap" style="margin-left: -10px;">
                                <i class="ti ti-dots nav-small-cap-icon fs-4"></i>
                        ...
                        @endif
                        @php
                            // Tentukan halaman yang sesuai berdasarkan permission
                            $defaultMasterRoute = '#'; // default fallback
                            $permissions = session('user_permissions', []);

                            if (in_array('view data jenis gabah', $permissions)) {
                                $defaultMasterRoute = '/jenis_gabah';
                            } elseif (in_array('view data perangkat', $permissions)) {
                                $defaultMasterRoute = '/data_device';
                            } elseif (in_array('view role', $permissions)) {
                                $defaultMasterRoute = '/role_manage';
                            }
                        @endphp

                        @if (in_array('view data jenis gabah', $permissions) ||
                                in_array('view data perangkat', $permissions) ||
                                in_array('view role', $permissions) || 
                                in_array('view data warehouses', $permissions) ||
                                in_array('view data bed dryer', $permissions))
                            <li class="sidebar-item">
                                <a class="sidebar-link {{ request()->is('jenis_gabah') || request()->is('data_device') || request()->is('role_manage') || request()->is('warehouse') || request()->is('bed_dryer') ? 'active' : '' }}"
                                    href="{{ $defaultMasterRoute }}" aria-expanded="false">
                                    <span>
                        ...
                        @if (in_array('view data pemesanan alat', session('user_permissions', [])))
                            <li class="sidebar-item">
                                <a class="sidebar-link {{ request()->is('pesanan*') ? 'active' : '' }}"
                                    href="/pesanan" aria-expanded="false">
                                    <span>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="27" height="27" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="w-5 h-5 transition-colors duration-200">
                                            <path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4a2 2 0 0 0 1-1.73z"/>
                                            <polyline points="3.27 6.96 12 12.01 20.73 6.96"/>
                                            <line x1="12" y1="22.08" x2="12" y2="12"/>
                                        </svg>
                                    </span>
                                    <span class="hide-menu">Data Pemesanan Alat</span>
                                </a>
                            </li>
                        @endif

                        @if (in_array('view riwayat pengeringan', session('user_permissions', [])))
                            <li class="sidebar-item">
                                <a class="sidebar-link {{ request()->is('riwayat*') ? 'active' : '' }}"
                                    href="/riwayat" aria-expanded="false">
                                    <span>
                        ...
                        @if (in_array('view informasi umum', session('user_permissions', [])))
                            <li class="nav-small-cap" style="margin-left: -10px;">
                                <i class="ti ti-dots nav-small-cap-icon fs-4"></i>
                                <span class="hide-menu">Lainnya</span>
                            </li>
                            <li class="sidebar-item">
                                <a class="sidebar-link {{ request()->is('informasi-umum') ? 'active' : '' }}"
                                    href="/informasi_umum" aria-expanded="false">
                                    <span>
                                        <i class="ti ti-settings" style="width: 24px;"></i>
                                    </span>
                                    <span class="hide-menu">Informasi Umum</span>
                                </a>
                            </li>
                        @endif
                        {{-- @if (in_array('view data sensor', session('user_permissions', [])))
        <li class="sidebar-item">
            <a class="sidebar-link {{ request()->is('data_sensor') ? 'active' : '' }}" href="/data_sensor" aria-expanded="false">
                <span>

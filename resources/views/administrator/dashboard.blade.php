...
function startWebSocket(dryerId) {
    ...
    if (!dryerId || dryerId === 'default') {
        ...
        showError('Dryer ID tidak valid. Menggunakan default channel.');
    }
    ...
    console.log('Starting WebSocket for channel:', `drying-process.${dryerId}`);

    window.Echo.channel(`drying-process.${dryerId}`)
        .subscribed(() => {
            console.log(`✅ SUBSCRIBED to drying-process.${dryerId}`);
            showSuccess(`Tersubskripsi ke channel drying-process.${dryerId}`);
        })
        .listen('.sensor-updated', (event) => {
            ...
            if (!data.sensors || !data.sensors.data || !Array.isArray(data.sensors.data) || data.sensors.data.length === 0) {
                ...
                showError('Data sensor kosong atau tidak valid dari WebSocket. Menunggu data berikutnya.');
                return;
            }

            // Ambil dryer_id dari drying_process jika tersedia
            const dryerId = data.drying_process?.dryer_id || currentDryerId || 'default';
            ...
            // Ambil data sensor terbaru untuk chart (gunakan data pertama atau rata-rata)
            const latestSensor = data.sensors.data[0]; // Ambil data sensor pertama
            if (!latestSensor) {
            ...
            showSuccess('Data sensor diperbarui secara real-time');
            hideError();

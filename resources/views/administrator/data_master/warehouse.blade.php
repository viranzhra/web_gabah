...
@section('content')
    ...
    <div class="card mt-4">
        <div class="card-body">
            <h5 class="fw-semibold mb-3">Data Warehouse</h5>
        ...
        <div class="modal-dialog" style="max-width: 330px;">
            <div class="modal-content">
                <div class="modal-header justify-content-center">
                    <h5 class="modal-title">Form Edit Warehouse</h5>
                ...
                <div class="modal-body" style="padding: 0 27px 27px 27px;">
                    <form id="editForm" method="POST">
                        ...
                        <div class="mb-3">
                            <label for="edit_nama" class="form-label">Nama Warehouse</label>
    ...
    <div class="modal fade" id="tambahDataModal" tabindex="-1" aria-labelledby="tambahDataLabel" aria-hidden="true">
        <div class="modal-dialog" style="max-width: 330px;">
            <div class="modal-content">
                <div class="modal-header justify-content-center">
                    <h5 class="modal-title" id="tambahDataLabel">Form Tambah Warehouse</h5>
                    <button type="button" class="btn-close position-absolute end-0 top-0 mt-3 me-3"
                        data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body" style="padding: 0 27px 27px 27px;">
                    <form id="tambahForm" method="POST">
                        @csrf
                        <div class="mb-3">
                            <label for="tambah_nama" class="form-label">Nama Warehouse</label>
    ...
    columns: [
        ...
        { data: 'deskripsi', defaultContent: '-' },
        {
            data: null,
            render: row => `
                <button onclick="editData(${row.warehouse_id})" class="btn btn-sm">
                    <i class="fas fa-edit" style="color: green; font-size: 18px;"></i>
                </button>
                <button onclick="deleteData(${row.warehouse_id})" class="btn btn-sm">
                    <i class="fas fa-trash-restore" style="color: #b60303; font-size: 18px;"></i>
                </button>`
        }
    ],
    language: {
        emptyTable: "Tidak ada data warehouse yang tersedia."
    }
});
                ...
                if (tambahModalEl) {
                    ...
                    $('#tambahForm').on('submit', function(e) {
                        ...
                        $.ajax({
                            url: "{{ config('services.api.base_url') }}/warehouses/store",
                            method: "POST",
                            headers: {
                                "Authorization": `Bearer ${sanctumToken}`,
                ...
                window.editData = function(id) {
                    $.ajax({
                        url: "{{ config('services.api.base_url') }}/warehouses/" + id,
                        method: "GET",
                        headers: {
                            "Authorization": `Bearer ${sanctumToken}`,
                            "Accept": "application/json",
                        ...
                        success: function(res) {
                            if (res && res.status && res.data) {
                                $("#edit_warehouse_id").val(res.data.warehouse_id);
                        ...
                        error: function(xhr) {
                            ...
                            if (xhr.status === 404) msg = "Warehouse tidak ditemukan.";
                            else if (xhr.status === 401) msg =
                ...
                $("#editForm").on("submit", function(e) {
                    e.preventDefault();
                    const id = $("#edit_warehouse_id").val();
                    const payload = {
                        nama: $("#edit_nama").val(),
...
window.deleteData = function(id) {
    // Ambil nama warehouse dari DataTables untuk ditampilkan di modal
    const rowData = table.rows().data().toArray().find(r => r.warehouse_id === id);
    const nama = rowData ? rowData.nama : "(Tidak diketahui)";

    deleteWarehouseId = id; // simpan id di variabel global sementara
    $("#deleteItemName").text(nama); // set nama di modal
...
$("#confirmDeleteBtn").on("click", function() {
    ...
    $.ajax({
        url: "{{ config('services.api.base_url') }}/warehouses/" + deleteWarehouseId,
        method: "DELETE",
        headers: {
            "Authorization": `Bearer ${sanctumToken}`,
            "Accept": "application/json",
        ...
        success: function() {
            table.ajax.reload(null, false);
            showNotification("success", "Berhasil!", "Warehouse berhasil dihapus.");
        },
        error: function(xhr) {
            console.error("AJAX Error:", xhr);
            let msg = xhr.responseJSON?.message || "Gagal menghapus warehouse.";
            if (xhr.status === 404) msg = "Warehouse tidak ditemukan.";
